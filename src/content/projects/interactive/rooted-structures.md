...
process:
  - "Prototyped a Stream Diffusion real-time AI pipeline, then pivoted to a Kinect point-cloud system when the hardware couldn't sustain live inference."
...
challenges: "The visual system was first built around Stream Diffusion, a real-time AI image-generation technique, but the available hardware couldn't hit the inference speed a live show needs. Instead of stalling, the team split the work: Andrei Stanciulescu rewrote the code to run the AI model inside TouchDesigner, while I rebuilt the visuals around a Kinect point-cloud system. The goal was a texture that feels fluid and organic against an industrial aesthetic, the rigid 'system' forced to react to the dancers' movement and care."
